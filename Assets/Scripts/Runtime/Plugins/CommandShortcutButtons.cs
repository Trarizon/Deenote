        ...
        public CommandShortcutButtons()
        {
            Plugins = ImmutableArray.Create(
                ImmutableArray.Create<IDeenotePlugin>(
                    new DelegatePlugin("Undo", new[] { ("zh", "撤销") }, context => { context.Editor.OperationMemento.Undo(null); return default; }),
                    new DelegatePlugin("Redo", new[] { ("zh", "重做") }, context => { context.Editor.OperationMemento.Redo(null); return default; })),
                ImmutableArray.Create<IDeenotePlugin>(
