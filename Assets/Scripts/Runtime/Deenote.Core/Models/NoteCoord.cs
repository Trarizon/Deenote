...
namespace Deenote.CoreB.Models
{
    ...
    public struct NoteCoord
    {
        ...
        public static NoteCoord ClampTime(NoteCoord coord, float maxTime)
            => coord with { Time = NoteConstraints.ClampTime(coord.Time, maxTime) };

        public static NoteCoord ClampPosition(float position, float time)
            => new(NoteConstraints.ClampPosition(position), time);

        public static NoteCoord Clamp(NoteCoord coord, float maxTime)
            => new(NoteConstraints.ClampPosition(coord.Position), NoteConstraints.ClampTime(coord.Time, maxTime));
