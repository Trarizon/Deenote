...
namespace Deenote.Core.Editing
{
    public sealed partial class StageNotePlacer : FlagNotifiable<StageNotePlacer, StageNotePlacer.NotificationFlag>
    {
        ...
        public StageNotePlacer(StageChartEditor editor, GameStageContext gameStageContext)
        {
            ...
            _editor._game.StageLoaded += args =>
            {
                ...
                var indicators = new PooledObjectListView<PlacementNoteIndicatorController>(
                    UnityUtils.CreateObjectPool(_stageContext.ThemeContext.CurrentTheme.ThemeConfig.NoteIndicatorPrefab,
                        _indicatorPanelTransform,
                        item => item.OnInstantiate(_stageContext.GameStage!.IndicatorPlane, _stageContext)));
            ...
            _gamePlayContext.TimeChanged += (s, args) =>
            {
        ...
        {
            var moveCoord = _grids.Quantize(NoteCoord.ClampPosition(coord), SnapToPositionGrid, SnapToTimeGrid);
        ...
        private partial StateFlag BeginPlaceSingleNote(NoteCoord coord, Vector2 mousePosition)
        {
            ...
            _beginNoteCoord = _grids.Quantize(coord, SnapToPositionGrid, SnapToTimeGrid);
        ...
        PlaceHold:
            ...
            var dragEndCoordTime = _grids.Quantize(coord, false, SnapToTimeGrid).Time;
        ...
        private partial StateFlag EndDragPlaceSlides(NoteCoord coord, Vector2 mousePosition)
        {
            ...
            _editor._editor.AddNotes(notes);
        ...
        private partial void UpdatePasteNotes(NoteCoord coord, Vector2 mousePosition)
        {
            ...
            if (PasteRememberPositionModifier) {
                ...
                coord = _grids.Quantize(coord, false, SnapToTimeGrid);
            }
            else {
                coord = _grids.Quantize(NoteCoord.ClampPosition(coord), SnapToPositionGrid, SnapToTimeGrid);
            }
        ...
        private partial StateFlag EndPasteNotes(NoteCoord coord, Vector2 mousePosition)
        {
            ...
            if (PasteRememberPositionModifier) {
                ...
                placeCoord = _grids.Quantize(placeCoord, false, SnapToTimeGrid);
            }
            else {
                placeCoord = _grids.Quantize(coord, SnapToPositionGrid, SnapToTimeGrid);
            }
            ...
            _editor._editor.AddNotes(_editor.ClipBoard.Notes);
        ...
        private void MoveIndicatorsTo(NoteCoord coord)
        {
            ...
            foreach (var indicator in _indicators) {
                ...
                c = NoteCoord.Clamp(c,_gamePlayContext.MusicLength);
                indicator.MoveTo(c);
            }
        }
        ...
        private bool IsInPlacementTime(NoteCoord coord)
        {
            var game = _editor._game;
            //game.AssertStageLoaded();
            return coord.Time <= _gamePlayContext.CurrentTime + _stageContext.GameStage.NoteAppearAheadTime;
        }
