...
namespace Deenote.GameStage.UI.Deemo
{
    ...
    public sealed class DeemoForegroundPerspectiveViewUI : ForegroundPerspectiveViewUI
    {
        ...
        private GamePlayContext _gamePlay;
        private GamePlayManagerB _gamePlayManager;
        ...
        private Difficulty Difficulty
        {
            ...
            set {
                if (Utils.SetField(ref _difficulty_bf, value)) {
                    var data = base.Config.Get(value);
                    _difficultyImage.sprite = data.IconSprite;
                    _levelText.color = data.TextColor;
                    UpdateLevelText();
                }
            }
        }
        ...
        protected override void Awake()
        {
            ...
            _gamePlay = MainSystem.Contexts.GamePlay;
            _gamePlayManager = MainSystem.GamePlayManagerB;
        ...
        private void Start()
        {
            _timeSlider.onValueChanged.AddListener(val => _gamePlay.CurrentTime = val);
            _pauseButton.onClick.AddListener(() => _gamePlayManager.TogglePlayingState());

            _gamePlay.RegisterPropertyChangedAndInvoke((s, e) =>
            {
                if (e.MatchProperty(nameof(s.MusicLength))) {
                    _timeSlider.maxValue = s.MusicLength;
                }
                if (e.MatchProperty(nameof(s.CurrentTime))) {
                    _timeSlider.SetValueWithoutNotify(s.CurrentTime);
                }
            });
            ...
            _project.RegisterNestedPropertyChangedAndInvokeNullable(s => s.CurrentChart, nameof(_project.CurrentChart), (s, e) =>
            {
                if (e.MatchProperty(nameof(s.Level))) {
                    Level = s?.Level ?? "";
                }
                if (e.MatchProperty(nameof(s.Difficulty))) {
                    Difficulty = s?.Difficulty ?? Difficulty.Easy;
                }
            });
