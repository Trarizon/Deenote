...
namespace Deenote.GameStage.Stage
{
    internal abstract class GameStageNoteController : MonoBehaviour
    {
        ...
        internal void OnInstantiate(GameStageContext context, GameStageNotePlaneController plane)
        {
            _context = context;
            _plane = plane;
        ...
        private void _OnPerspectiveLineCollecting(PerspectiveLinesRenderer.LineCollector collector)
        {
            if (_linkLine is var (start, end)) {
                //_game.AssertStageLoaded();
                var gridLineArgs = _context.ThemeContext.CurrentTheme.GridLineConfig;
                collector.AddLine(start, end,
                    gridLineArgs.LinkLineColor with { a = _noteColorAlpha },
                    gridLineArgs.LinkLineWidth);
            }
        }
        ...
        private NotePlayState GetState()
        {
            ...
            bool IsInvisible()
            {
                ...
                if (!_context.IsEarlyDisplaySlowNotes) {
                    ...
                    if (_context.NotesContext.GetNextActiveNodeInTimeOrderDisplayMode() is { } next) {
        ...
        {
            _stageDeltaTime = NoteModel.Time - _context.NotesContext.CurrentTime;
            if (_stageDeltaTime > AppearAheadTime) {
                goto Invisible;
            }
            if (!_context.IsEarlyDisplaySlowNotes &&
                // In TimeOrder mode, the note should display only after its previous note displayed
                _context.NotesContext.GetNextActiveNodeInTimeOrderDisplayMode() is { } next &&
        ...
        {
            var relativeTime = _context.NotesContext.CurrentTime - NoteModel.Time;
        ...
        private void SetHighlightState()
        {
            ...
            if (_context.IsNoteDownplayed(NoteModel))
        ...
        {
            var time = _context.NotesContext.CurrentTime - NoteModel.EndTime;
        ...
        {
            transform.WithLocalPositionX(_context.GameStage!.ConvertNotePositionToWorldX(NoteModel.Position));
        }
        ...
        private void SetLinkLine()
        {
            ...
            if (_playState is NotePlayState.Fall && _context.IsShowLinkLines && NoteModel.NextLink is not null) {
