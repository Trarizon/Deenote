...
{
    internal static class ChartJsonAdapter
    {
        ...
        public static ChartData? ParseDeV3Json(string json)
        {
            ...
            var links = chart.links.Select(link => new NoteLinkIterator.Deserializer(link.notes.Select(
                nref =>
                {
                    ...
                    return notes[index];
                }) ?? Enumerable.Empty<NoteData>()));
            return new ChartData(chart.speed, oriVMin: 0, oriVMax: 0, remapVMin: 10, remapVMax: 70,
                notes, links, null);
        }
