...
{
    internal sealed class ProjectSerializer
    {
        public static ProjectSerializer Instance { get; } = new ProjectSerializer();

        public const byte FileVersionMark = 1;

        public async UniTask<ProjectModel?> DeserializeAsync(FileStream stream, CancellationToken cancellationToken = default)
        {
            ...
            var project = await ReadProjectAsync(reader, cancellationToken);
            return project!;
        }

        public async UniTask SerializeAsync(ProjectModel project, string path, CancellationToken cancellationToken = default)
        {
            ...
            try {
                using (var fs = File.OpenWrite(tmpPath)) {
                    ...
                    await UniTask.RunOnThreadPool(() => WriteProject(writer, project, cancellationToken));
                }
        ...
        private static async UniTask<ProjectModel> ReadProjectAsync(BinaryReader reader, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
